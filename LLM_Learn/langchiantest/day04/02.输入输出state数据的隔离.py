...
# 创建langgraph的构建器
builder = StateGraph(state_schema=MyState,input_schema=InputState,output_schema=OutputState)  # 创建一个状态图的构建器,内部会创建状态的字典吗，就是节点的第一个形参state接受的值
...
res = graph.invoke({"query":"用户问的问题","web_search_result":"哈哈"})
